...
\end{defi}
    ...
    A \Cref{agb-def-parOrdenado} é satisfatória para esse objetivo, mas para nós o que irá importar é o lema a seguir:
...
\end{defi}
O conjunto quociente serve para particionar o conjunto em subconjuntos distintos. No \Cref{agb-ex-classeEquivalenciaDivisaoEuclidiana} consideramos a divisão por $4$ no conjunto dos números naturais. Notamos que formamos $4$ classes de equivalência distintas, e qualquer número natural pertence à uma, e apenas uma classe de equivalência.

\begin{ex}
    No \Cref{agb-ex-classeEquivalenciaDivisaoEuclidiana}, se formos utilizar essa notação, usamos:
...
% Por outro lado, exemplos e considerações podem não ser totais. 
...
\end{defi}
\begin{ex}
    No \Cref{agb-ex-cotasSuperiorInferior} o $1$ é mínimo e o $5$ é máximo. O $6$ é uma cota superior mas não é máximo de $A$ pois $6 \not\in A$.
\end{ex}
...
\end{teo} 
\begin{dem}
    Sejam $\leq$ uma relação de ordem sobre $A$, e $M_1$ e $M_2$ dois máximos de $A$. Temos $M_1, M_2 \in A$, e também $M_1 \leq M_2$  e $M_2 \leq M_1$. Como uma relação de ordem é antissimétrica, na \Cref{agb-def-relacaoOrdemParcial}, concluí-se que $M_1 = M_2$.
\end{dem}
...
\begin{obs}
    Neste trabalho em geral trabalharemos com duas operações principais para cada um dos conjuntos $\mathbb{N}, \mathbb{Z}, \mathbb{Q}$ e $\mathbb{R}$. Para cada um desses conjuntos haverá uma adição (soma) e uma multiplicação (produto). O elemento simétrico da \Cref{agb-def-propriedades} continuará sendo chamado de simétrico caso seja uma adição, e passará a ser chamado de inverso, quando se tratar de multiplicação.
\end{obs}
...
\end{teo}
\begin{dem}
    Se $a = 0$ ou se $b=0$ então $ab = 0$ pela \Cref{agb-teo-anulamentoProduto1}.
